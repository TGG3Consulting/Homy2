...
function DemoPropertyCard({ property, visible, isBest, index }) {
  const scoreDisplay = AnimatedCounter(property.score, visible);
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isBest
          ? "0 0 0 3px rgba(139,108,255,0.08), 0 4px 20px rgba(139,108,255,0.06)"
          ...
          <div>
            <div className="flex items-start justify-between gap-1.5">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                {isBest && <Star size={10} fill="#8B6CFF" style={{ color: "#8B6CFF" }} />}
                <span className="text-[12px] font-body font-bold" style={{ color: isBest ? "#8B6CFF" : "#999999" }}>
                  {scoreDisplay}%
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 mt-1 text-[10px] font-body" style={{ color: "#999999" }}>
              <span className="flex items-center gap-1"><Bed size={9} style={{ color: "#8B6CFF" }} />{property.rooms} rooms</span>
              <span className="flex items-center gap-1"><Maximize2 size={9} style={{ color: "#8B6CFF" }} />{property.area} m²</span>
            </div>
          </div>
...
export default function ForRenters() {
      ...
      return (
        <div className="flex justify-end">
          <div
            ...
            style={{ backgroundColor: "rgba(139,108,255,0.08)", border: "1px solid rgba(139,108,255,0.1)" }}
          >
    ...
    if (activeStep === 1) {
      return (
        <div className="flex items-start gap-3">
          <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "rgba(139,108,255,0.1)" }}>
            <Sparkles size={12} style={{ color: "#8B6CFF" }} />
          </div>
          <div className="rounded-2xl px-5 py-3.5" style={{ backgroundColor: "rgba(255,255,255,0.6)", border: "1px solid rgba(200,196,188,0.3)" }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {(t("forRentersPage.demoChips") || ["Budget", "District", "Children", "Pets", "School", "Commute"]).map((chip) => (
                <span key={chip} className="px-2.5 py-1 rounded-full text-[10px] font-body font-medium"
                  style={{ backgroundColor: "rgba(139,108,255,0.08)", color: "#8B6CFF" }}>
                  {chip}
                </span>
              ))}
            </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="rounded-2xl px-5 py-3.5 flex items-center gap-3"
            ...
            <div className="w-4 h-4 border-2 rounded-full animate-spin flex-shrink-0"
              style={{ borderColor: "rgba(139,108,255,0.2)", borderTopColor: "#8B6CFF" }} />
      ...
      return (
        <div className="space-y-2">
          ...
          {showCostBreakdown && (
            ...
            >
              <p className="text-[10px] font-body font-semibold uppercase tracking-wider mb-2.5" style={{ color: "#8B6CFF" }}>
              ...
              <div className="space-y-1.5">
                {costBreakdown.map((item, i) => (
                  <div key={i} className="flex items-center justify-between">
                    ...
                    <span className="text-[12px] font-body font-semibold" style={{ color: item.highlight ? "#8B6CFF" : "#242424" }}>
                      {item.value}
                    </span>
                  </div>
                ))}
  ...
  return (
    <div className="w-full min-h-screen" style={{ background: `radial-gradient(ellipse at 50% 30%, #F8F7F5 0%, ${BG} 70%)` }}>
      {/* ── Top bar ── */}
      <div className="relative z-10 flex items-center justify-between px-7 pt-5 pb-2 max-w-7xl mx-auto">
        <Link to="/" className="font-display font-semibold tracking-[0.15em] uppercase select-none"
          style={{ fontSize: "15px", color: "#242424" }}>
          Homly
        </Link>
        <div className="flex items-center gap-4">
          <Link to="/for-buyers" className="text-[13px] font-body font-medium transition-opacity hover:opacity-60" style={{ color: "#666" }}>
            {t("common.buy")}
          </Link>
          <Link to="/for-owners" className="text-[13px] font-body font-medium transition-opacity hover:opacity-60" style={{ color: "#666" }}>
            {t("common.owners")}
          </Link>
          <Link to="/" className="inline-flex items-center gap-1.5 px-5 py-2 rounded-full text-[13px] font-body font-semibold transition-all duration-200 hover:shadow-md"
            style={{ backgroundColor: "#8B6CFF", color: "#FFF" }}>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20">
        ...
        <div className="text-center pt-12 pb-10">
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            <Link to="/" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-[14px] font-body font-semibold transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5"
              style={{ backgroundColor: "#8B6CFF", color: "#FFF" }}>
              {t("forRentersPage.cta")}
              <ArrowRight size={16} />
            </Link>
            <Link to="/how-it-works" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-[14px] font-body font-medium transition-all duration-200 hover:shadow-sm"
              style={{ backgroundColor: "rgba(255,255,255,0.5)", color: "#8B6CFF", border: "1px solid rgba(139,108,255,0.15)" }}>
              {t("forRentersPage.howHomlyWorks")}
            </Link>
          </div>
          <p className="mt-4 inline-block px-5 py-2 rounded-full font-body font-medium"
            style={{ fontSize: "13px", color: "#8B6CFF", backgroundColor: "rgba(139,108,255,0.06)", border: "1px solid rgba(139,108,255,0.12)" }}>
              ...
              <div className="w-[210px] flex-shrink-0 border-r p-4 space-y-0.5" style={{ borderColor: "rgba(200,196,188,0.15)", backgroundColor: "rgba(255,255,255,0.3)" }}>
                {leftSteps.map((label, i) => (
                  <div key={i} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-all duration-500"
                    style={{
                      backgroundColor: i === activeStep ? "rgba(139,108,255,0.06)" : "transparent",
                      border: i === activeStep ? "1px solid rgba(139,108,255,0.15)" : "1px solid transparent",
                    }}>
                    <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-body font-semibold transition-all duration-500"
                      style={{
                        backgroundColor: i === activeStep ? "#8B6CFF" : i < activeStep ? "rgba(139,108,255,0.2)" : "rgba(36,36,36,0.05)",
                        color: i === activeStep ? "#FFF" : i < activeStep ? "#8B6CFF" : "#BBBBBB",
                      }}>
                      {i < activeStep ? "✓" : i + 1}
                    </div>
                ...
                <div className="flex items-center gap-2 mt-5 justify-center">
                  {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
                    <button key={i} onClick={() => setActiveStep(i)} className="rounded-full transition-all duration-300"
                      style={{ width: i === activeStep ? 22 : 5, height: 5, backgroundColor: i === activeStep ? "#8B6CFF" : "rgba(36,36,36,0.12)" }} />
                  ))}
                </div>
              </div>
            </div>
            ...
            {features.map((feature, i) => (
              <div key={i} className="rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1"
                ...
                <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-3.5"
                  ...
                  <feature.icon size={20} style={{ color: "#8B6CFF" }} />
                </div>
          ...
          <div className="grid grid-cols-2 gap-6 max-w-[720px] mx-auto">
            <div className="rounded-2xl p-6"
              ...
              <ul className="space-y-3">
                {(Array.isArray(comparison.traditional) ? comparison.traditional : []).map((item, i) => (
            ...
            <div className="rounded-2xl p-6"
              style={{
                ...
                boxShadow: "0 0 0 2px rgba(139,108,255,0.03)",
              }}>
              <p className="text-[12px] font-body font-semibold uppercase tracking-wider mb-4" style={{ color: "#8B6CFF" }}>
                {t("forRentersPage.homlyLabel")}
              </p>
              <ul className="space-y-3">
                {(Array.isArray(comparison.homly) ? comparison.homly : []).map((item, i) => (
                  <li key={i} className="flex items-start gap-2">
                ...
                {(Array.isArray(checklistItems) ? checklistItems : []).map((item, i) => (
                  <div key={i} className="flex items-start gap-2.5">
                    <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-[1px]"
                      ...
                      <ShieldCheck size={10} style={{ color: "#8B6CFF" }} />
                    </div>
                    <span className="text-[13px] font-body leading-snug" style={{ color: "#3D3B37" }}>{item}</span>
                  </div>
                ))}
        ...
        <div className="text-center">
          <Link to="/" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full text-[16px] font-body font-semibold transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5"
            style={{ backgroundColor: "#8B6CFF", color: "#FFF" }}>
            {t("forRentersPage.cta")}
            <ArrowRight size={18} />
          </Link>
