...
function MiniCard({ p, visible, best, counting, idx }: MiniCardProps) {
  ...
  return (
    <div
      ...
      style={{
        ...
        border: best ? "1px solid rgba(10, 96, 69,0.3)" : "1px solid rgba(200,196,188,0.2)",
        boxShadow: best ? "0 0 0 3px rgba(10, 96, 69,0.08)" : "0 2px 6px rgba(0,0,0,0.03)",
      ...
      <div className="flex-1 min-w-0 px-3 py-2">
        <div className="flex items-start justify-between gap-1">
          ...
          <div className="flex items-center gap-0.5 flex-shrink-0">
            {best && <Star size={9} fill="#0A6045" style={{ color: "#0A6045" }} />}
            <span className="text-[11px] font-body font-bold" style={{ color: best ? "#0A6045" : "#999" }}>{score}%</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mt-1 text-[10px] font-body" style={{ color: "#999" }}>
          <span><Bed size={8} style={{ display: "inline", marginRight: 2, color: "#0A6045" }} />{p.rooms} rooms</span>
          <span><Maximize2 size={8} style={{ display: "inline", marginRight: 2, color: "#0A6045" }} />{p.area} m²</span>
        </div>
    ...
    if (step === 0) {
      return (
        <div className="flex justify-end">
          <div className="max-w-[78%] rounded-2xl rounded-br-md px-4 py-3" style={{ backgroundColor: "rgba(10, 96, 69,0.08)", border: "1px solid rgba(10, 96, 69,0.1)" }}>
    ...
    if (step === 1) {
      return (
        <div className="flex items-start gap-2.5">
          <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "rgba(10, 96, 69,0.1)" }}>
            <Sparkles size={10} style={{ color: "#0A6045" }} />
          </div>
          <div className="rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(255,255,255,0.6)", border: "1px solid rgba(200,196,188,0.3)" }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {(Array.isArray(criteriaChips) ? criteriaChips : ["Budget: $85k", "3 bedrooms", "Yerevan", "Near school", "Buy"]).map((c: string) => (
                <span key={c} className="px-2 py-0.5 rounded-full text-[9px] font-body font-medium" style={{ backgroundColor: "rgba(10, 96, 69,0.08)", color: "#0A6045" }}>{c}</span>
              ))}
            </div>
          </div>
        </div>
    ...
    if (step >= 2) {
      return (
        <div className="space-y-2">
          ...
          {step >= 3 && (
            <div className="rounded-lg px-3.5 py-2.5 mt-2" style={{ backgroundColor: "rgba(10, 96, 69,0.04)", border: "1px solid rgba(10, 96, 69,0.12)" }}>
              <p className="text-[9px] font-body font-semibold uppercase tracking-wider mb-1" style={{ color: "#0A6045" }}>{t("buyerDemo.districtsCompared")}</p>
          ...
          {step >= 5 && (
            <div className="rounded-lg px-3.5 py-2.5" style={{ backgroundColor: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.12)" }}>
              ...
              <div className="space-y-1">
                {(Array.isArray(risks) ? risks : ["Building year: 1985 — check structural report", "Ownership: 1 owner — clean title"]).map((r: string, i: number) => (
            ...
            <div className="rounded-lg p-3.5 mt-1" style={{ backgroundColor: "rgba(255,255,255,0.7)", border: "1px solid rgba(200,196,188,0.3)", boxShadow: "0 2px 10px rgba(0,0,0,0.04)" }}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-1.5">
                  <Clock size={12} style={{ color: "#0A6045" }} />
                  <span className="text-[12px] font-body font-semibold" style={{ color: "#242424" }}>{t("buyerDemo.viewingAvailable")}</span>
                </div>
                <span className="text-[9px] font-body font-semibold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: "rgba(10, 96, 69,0.1)", color: "#0A6045" }}>{t("buyerDemo.new")}</span>
              </div>
              ...
              <button className="w-full py-2 rounded-lg text-[12px] font-body font-semibold" style={{ backgroundColor: "#0A6045", color: "#FFF" }}>
    ...
    >
      ...
      <div className="flex" style={{ minHeight: "380px" }}>
        ...
        <div className="w-[200px] flex-shrink-0 border-r p-3.5 space-y-0.5" style={{ borderColor: "rgba(200,196,188,0.15)", backgroundColor: "rgba(255,255,255,0.3)" }}>
          {(Array.isArray(sidebarLabels) ? sidebarLabels : ["You describe your need","Criteria extracted","Properties found","Districts compared","Price check","Risks analyzed","Schedule viewing"]).map((label: string, i: number) => (
            <div
              key={i}
              className="flex items-center gap-2 px-2 py-1.5 rounded-md transition-all duration-500"
              style={{
                backgroundColor: i === step ? "rgba(10, 96, 69,0.06)" : "transparent",
                border: i === step ? "1px solid rgba(10, 96, 69,0.15)" : "1px solid transparent",
              }}
            >
              <div
                ...
                style={{
                  backgroundColor: i === step ? "#0A6045" : i < step ? "rgba(10, 96, 69,0.2)" : "rgba(36,36,36,0.05)",
                  color: i === step ? "#FFF" : i < step ? "#0A6045" : "#BBB",
                }}
              >
                {i < step ? "✓" : i + 1}
              </div>
        ...
        <div className="flex-1 p-5 flex flex-col justify-center overflow-hidden" style={{ backgroundColor: "rgba(249,249,247,0.5)" }}>
          ...
          <div className="flex items-center gap-1.5 mt-5 justify-center">
            {Array.from({ length: TOTAL }).map((_, i) => (
              <button key={i} onClick={() => setStep(i)} className="rounded-full transition-all duration-300" style={{ width: i === step ? 20 : 4, height: 4, backgroundColor: i === step ? "#0A6045" : "rgba(36,36,36,0.12)" }} />
            ))}
          </div>
        </div>
      </div>
