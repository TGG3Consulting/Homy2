...
function SharePopover({ isOpen, onClose, property }: SharePopoverProps) {
  const [copied, setCopied] = useState(false);
  const link = `https://homy.am/property/${property.id}`;
        ...
        >
          {submitted ? (
            <div className="text-center py-6">
              ...
              <p className="text-[13px] font-body" style={{ color: "#999999" }}>Thank you for helping keep Homy accurate.</p>
            </div>
          ) : (
            <>
...
export default function PropertyActionBar({ property }: PropertyActionBarProps) {
          ...
          onClick={async () => {
            try {
              ...
              if (response.ok) {
                ...
                console.error('[PropertyActionBar] Failed to save favorite:', response.status);
              }
            } catch (err) {
              console.error('[PropertyActionBar] Error saving favorite:', err);
            }
          }}
