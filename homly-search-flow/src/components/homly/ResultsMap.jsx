...
function MapUpdater({ selectedId, properties }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // map not fully initialized, skip animation
    }
...
export default function ResultsMap({ properties, selectedId, fullBg }) {
  ...
  return (
    <div className="w-full h-full min-h-[300px] overflow-hidden" style={{ backgroundColor: "#EDE9E1" }}>
      <MapContainer
      ...
      >
        ...
        {selectedProp && !isNaN(Number(selectedProp.latitude)) && !isNaN(Number(selectedProp.longitude)) && (
          <>
            <Circle
